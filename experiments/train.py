...
from utils.logger import MLflowOutputFormat, log_agent_params, log_env_params, log_net_params, log_rew_params
...
def main(args):
    agent_config, env_config, reward_config, net_arch, train_config = parse_config(args)
    env = generate_env_from_config(env_config, reward_config)
    eval_env = Monitor(generate_env_from_config(env_config, reward_config))
    agent = generate_agent(env, agent_config, net_arch)
